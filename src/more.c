...
/*   Updated: 2019/09/19 17:04:17 by bperez      ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
void	display_hud(t_fdf *fdf)
{
	...
	put_string(fdf, 105, HEIGHT - 190, ft_itoa(fdf->map->total));
	put_string(fdf, 20, HEIGHT - 165, "rows  = ");
	put_string(fdf, 105, HEIGHT - 165, ft_itoa(fdf->map->rows));
	put_string(fdf, 20, HEIGHT - 140, "cols  = ");
	put_string(fdf, 105, HEIGHT - 140, ft_itoa(fdf->map->cols));
	put_string(fdf, 20, HEIGHT - 115, "zoom  = ");
	put_string(fdf, 105, HEIGHT - 115, ft_itoa(fdf->map->zoom));
	put_string(fdf, 20, HEIGHT - 90, "x_rot = ");
	put_string(fdf, 105, HEIGHT - 90, ft_itoa(fdf->map->x_rot));
	put_string(fdf, 20, HEIGHT - 65, "y_rot = ");
	put_string(fdf, 105, HEIGHT - 65, ft_itoa(fdf->map->y_rot));
	put_string(fdf, 20, HEIGHT - 40, "z_rot = ");
	put_string(fdf, 105, HEIGHT - 40, ft_itoa(fdf->map->z_rot));
}
